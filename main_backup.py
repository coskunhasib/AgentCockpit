            ...
            for line in f:
                if line.startswith('home = '):
                    ...
                    if not os.path.exists(python_home):
                        print(f"[BOOTSTRAP] Venv geçersiz: Python yolu bulunamadı -> {python_home}")
                        return False
                    return True
...
def create_venv_and_restart():
    ...
    if os.path.exists(venv_dir):
        if not is_venv_valid(venv_dir):
            print("[BOOTSTRAP] Taşınabilirlik: Venv farklı bir PC'de oluşturulmuş. Yeniden oluşturuluyor...")
            needs_recreate = True
            # Eski venv'i sil
            import shutil
            try:
    ...
    try:
        ...
        if os.path.exists(requirements_file):
            ...
            subprocess.check_call([
                venv_python, "-m", "pip", "install", "pip_system_certs",
                "--trusted-host", "pypi.org",
                "--trusted-host", "pypi.python.org",
                "--trusted-host", "files.pythonhosted.org"
